...
a **forest rooted at trunk**. A **stack line** is the linear chain from trunk up
...
- Branching a stack (two children on one branch) falls out naturally.
- Merge order is implied: bottom-first along a line.

Trade-off: a "stack" is not a first-class named object, so operations pick the
...
5. A conflict stops the run with instructions; re-running resumes cleanly
   because already-restacked branches are detected as up to date (anchor ==
...
For stack line `main ← A ← B ← C`:

- **Base chaining:** PR(A)→`main`, PR(B)→`A`, PR(C)→`B`. Each PR shows only its
  own diff.
- **Push order:** bottom-first, so every PR's base branch already exists on the
...
| `meta.rs` | Read/write stack metadata in git config. |
| `stack.rs` | Domain model: chains, forks, topological order. |
...
- **Integration tests** (`tests/integration.rs`): run the built binary against
  ...
  `untrack`, and the stacked `sync`. `submit` is excluded because it needs an
